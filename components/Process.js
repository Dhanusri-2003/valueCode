export default function Process() {
  const steps = [
   ...
   return (
    <section id="process" className="py-16 md:py-24 bg-[#F5F0ED]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {steps.map((step, index) => (
            <div key={index} className="relative">
              <div className="bg-white rounded-xl p-6 md:p-8 text-center border border-[#D7CCC8] hover:border-[#8B7355] hover:shadow-2xl hover:shadow-[#8B7355]/20 transition-all duration-300 h-full">
                {/* Step Number */}
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 w-10 h-10 bg-[#8B7355] text-white rounded-full flex items-center justify-center font-bold text-lg">
                  {step.number}
                </div>
                ...
                {/* Step Content */}
                <h3 className="text-lg md:text-xl font-semibold text-[#3E2723] mb-3 md:mb-4">{step.title}</h3>
                <p className="text-[#5D4037] leading-relaxed text-sm md:text-base">{step.description}</p>
              </div>
            </div>
          ))}
        </div>
